...
function Search() {
    ...
    <div className='relative text-color-title mb-3' >
        <span className="absolute top-3 max-sm:top-2 pl-2.5 left-0 text-color-search-icon"><FontAwesomeIcon icon={faSearch} /></span>
        <input className='w-full py-3 max-sm:py-2 pr-1 pl-9 rounded-xl outline outline-1 outline-slate-400 bg-color-search-user dark:bg-dark-color-input' value={searchValue} onChange={(e) => setSearchValue(e.target.value)} placeholder="Search user" />
        {loading === false ? 
        <span className="absolute top-3 max-sm:top-2 font-semibold mr-2.5 right-0 cursor-pointer text-color-search-icon hover:text-black dark:hover:text-white" onClick={handleDeleteValue}><FontAwesomeIcon icon={faClose} /></span>
        : 
