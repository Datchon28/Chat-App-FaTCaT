    ...
    return (
        <div id="all-chat" className="max-h-height-child-list-chat-sidebar h-full overflow-y-scroll mt-5">
            <ul className='list-none flex justify-center items-center flex-col -mt-3'>
                {room.map((list, index) => (
                <li id={list._id} onClick={(e) => handleOpenRoom(e.target)} key={index} className={`flex flex-col items-start justify-between my-3 px-3 py-2 w-full rounded-xl ${idRoomChoosing === list._id ? 'bg-color-seen dark:bg-dark-color-seen transition-colors' : 'bg-color-none-seen dark:bg-dark-color-none-seen'} font-semibold cursor-pointer`}>
