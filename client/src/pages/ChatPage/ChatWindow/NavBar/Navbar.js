...
function Navbar({ listChatAndRoom }) {
    return (
        <div className="nav-bar max-h-navbar h-full w-full">
        {listChatAndRoom.map((list, index) => (
            <div key={index} className="user-info border border-solid border-color-chat-window max-h-navbar h-full w-full flex items-center justify-between px-5 py-3 bg-color-content " >
